...
// Once a player has actually picked an era, that era's own background photo
// (the same file the home hero's era switcher uses) replaces the generic fog
// for the rest of that session -- game.html calls this instead of
// mountAtmosphereBackground(), never both, so the day loop feels like it's
// still inside that era's world rather than back to a neutral default.
// The dim/blur treatment matches the hero's layers exactly, since it was
// already verified there to keep real UI (a decision card) fully legible
// over a busy photo.
export function mountEraImageBackground(eraId) {
  if (document.getElementById('era-bg-image')) return;
  const div = document.createElement('div');
  div.id = 'era-bg-image';
  div.style.backgroundImage = `url('./assets/era-bg-${eraId}.jpg')`;
