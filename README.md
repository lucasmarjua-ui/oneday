...
assets/era-bg-*.jpg            Home hero's per-era background photographs
shared/
  ...
  card-art.js                  Per-era gradient palette + decorative gauge/connections SVG for option/archetype cards
  led-digits.js                Renders a cost/time string's digits as dot-matrix "LED" seven-segment glyphs
...
**One cinematic identity for the whole game.** A single dark-navy palette (`--background: hsl(201 100% 13%)`, white foreground, muted-gray secondary text) and one type pairing — **Instrument Serif** for every heading/display moment, **Inter** for body text — apply everywhere, from the home screen through character creation, the day loop, and every summary screen. Every card, button, badge and panel shares one surface treatment, `.liquid-glass`: a near-transparent `backdrop-filter: blur()` fill with a soft gradient border traced by a `::before` pseudo-element (`mask-composite: exclude` cutting a ring out of a linear gradient) rather than a flat 1px border — the same trick used for `.panel`, `.btn`, `.archetype-card`, `.option-card`, NPC/character portrait badges, and the chance-success ring, all defined together in `shared/theme.css` so no template needed an extra class for it. Status colors (the resource-bar green/amber/red, success/fail tags) are the one deliberate exception — those keep real meaning, only their container went glass.

Entrances use one `fade-rise` keyframe (`opacity 0→1`, `translateY(24px)→0`), reused for the hero copy, decision/archetype cards and summary reveals alike — one motion language instead of several per-screen animations.

### Decision and archetype cards: a glass grid, not a button list

Every point in the game where the player is choosing between 2-4 options — a decision card's options during the day loop, and archetype selection during character creation — renders as a CSS grid of `.option-card` / `.archetype-card` elements (`.options-grid` / `.archetype-grid`), not a vertical list of plain buttons. The grid's column count tracks how many options actually exist for that specific choice via a `--card-count` custom property set in JS (`optionsList.style.setProperty('--card-count', currentCard.options.length)`), so 2, 3 or 4 options always fill the row evenly instead of leaving gaps or forcing a fixed layout; it collapses to one column under `max-width: 640px` (options) / `720px` (archetypes).

Each card gets its own two-stop background gradient wash (`shared/card-art.js`'s `getCardGradient(eraId, index)`, applied via a `--card-gradient` custom property and painted by the card's `::after`, under the content but above the glass base) from one of three era palettes — amber/terracotta for Greece, ochre/fire-orange for Neanderthals, cyan/magenta neon for the Futuristic City — coherent with that era's own hero photo, and cycling by option index so no two cards in the same row share a gradient. The same module's `renderCardOrnamentSVG(seed)` draws a small, purely decorative SVG in the card's corner — alternating between a circular gauge (a partial ring + center dot, like a dial) and a five-point "connections map" (dots joined by thin lines) — deterministically varied per option/archetype id (a tiny string hash), so it's stable across re-renders of the same card but different from its neighbors. New cards mount fresh into the DOM on every decision, so each one's own `fade-rise` entrance animation plays from scratch automatically; a handful of `nth-child` rules give cards 1-4 in a row a small staggered delay (0s, 0.08s, 0.16s, 0.24s) so they enter as a cascade, not all at once.

An option's cost/time readout ("1h", "2h · Drachmas -5") renders through `shared/led-digits.js`'s `renderLedText()`, which maps each digit 0-9 to a small SVG glyph built from the same seven-segment layout as a classic digital display, except every segment is drawn as three small dots rather than a solid bar — a dot-matrix look instead of plain numerals. Anything in the string that isn't a digit (letters, `·`, `+`/`-`, spaces) renders as normal small text right alongside the glyphs, so only the actual numbers get the LED treatment.

The appearance screen's swatch buttons (base/outfit/headgear/accessory) keep their existing small-pill shape rather than becoming full cards — there's no cost or per-option art to show there — but a selected swatch's ring now glows in that era's accent color (`--accent-glow`, set via a `body[data-era="..."]` rule once an era is picked) instead of plain white, so the character-creation flow still reads as belonging to a specific era even where the glass-card system itself doesn't apply.

### One generated atmosphere, everywhere

Character creation sits over a generated backdrop: a handful of slow-drifting, low-opacity radial-gradient "fog" blobs plus one soft light halo, all in the site's own navy hue, rendered on a fixed, full-viewport `<canvas>` (`shared/atmosphere-bg.js`, `mountAtmosphereBackground()`) pinned behind all page content (`z-index: 0`, everything else in the game already sat at `z-index: 1` or higher for the critical-alert vignette, so no new stacking rules were needed) — one shared component, not a per-screen effect or something reimplemented on each page. `game.html` uses the same module's sibling function instead once a specific era is actually being played (see below), so the fog and the era background photo are never both mounted at once.

The canvas approach was chosen over a screen-recorded video loop for this shared backdrop specifically because it needed to work believably behind decision cards and the day loop too, not just one hero screen — a single looping video recorded *of* the era-select screen would have looked wrong playing behind gameplay it wasn't recorded of. It's also cheap at runtime: gradient *stops* fade to transparent instead of an expensive per-frame blur filter, the canvas is capped at 1.5x device pixel ratio, and the draw loop self-throttles to ~24fps — verified via Playwright's `PerformanceObserver('longtask')` API under iPhone 13 emulation with the animation running continuously: zero long tasks (0ms) over a 3-second sample, and UI clicks (era → archetype transition) still resolved in under 150ms. `prefers-reduced-motion: reduce` freezes it on a single static frame.

### The photo hero and era switcher

The hero band at the very top uses three real photographs, one per playable era (`assets/era-bg-{greece,neanderthal,future-city}.jpg`), stacked as full-bleed `<div>` background layers with only the active one at visible opacity (crossfaded via a 1000ms CSS `opacity` transition). The active layer also carries a slow 24s `scale(1)` → `scale(1.08)` zoom (`ease-in-out alternate infinite`), a small Ken-Burns-style motion cue so a static photo doesn't feel completely inert. Legibility comes from a single center-weighted radial-gradient scrim (`.hero-scrim`, darkest behind the headline, easing off toward the edges) sitting between the photos and the hero copy — not a filter on the images themselves, since these are real location/scene photography rather than a second, competing UI, so the images stay visually crisp everywhere the scrim doesn't need to darken them.

A row of era-switcher tabs beneath the hero copy — one per playable era, name only, active tab underlined — drives which background layer shows, debounced with a 1000ms cooldown so rapid clicks can't overlap transitions (verified directly in the browser: at every step exactly one `.hero-bg-layer` carries the `active` class, matched by a plain `data-era-bg === activeSwitcherEraId` string comparison, never by array index). The hero's own "Play Now" button (and the nav's matching pill) starts whichever era is currently showing directly via the existing `selectEra()` flow, into archetype selection — the photo hero *is* the era picker now, not a banner in front of one.

The old emoji-icon era-grid below the hero (separate "Play"/Today's Challenge cards per era) was removed as a duplicate of what the hero now does. What it covered that the hero's single "Play Now" doesn't — starting a specific era's *Daily Challenge*, not free play — became a small, honest `.liquid-glass`-styled picker instead: a "Play Today's Challenge in…" heading over three plain glass pill buttons, one per era, each calling the same `selectDailyChallenge(eraId)` this project already had.

The nav (logo, Home/How it works/Today's Challenge/Profile links — the last three now grouped in one glass pill per a later design pass — language toggle, login, and a solid "Play Now" pill, plus a glass hamburger toggle with a 300ms icon crossfade on narrow screens) is its own persistent header, shown on every phase of the home page, not nested inside the decorative hero band — an earlier structure that put it *inside* `.hero` meant the whole nav (including language and login) disappeared the moment you moved into character creation, since that section hides once an era is picked.

### The era's photo follows you into the day

Picking an era doesn't leave its photo behind on the home screen — `game.html` calls `mountEraImageBackground(eraId)` (the same `shared/atmosphere-bg.js` module, a sibling to the fog function, never both mounted together) to show that exact era's own background photo, dimmed and lightly blurred (`filter: blur(1px) saturate(0.8) brightness(0.7)`), as the backdrop for the whole day: the HUD, every decision card, outcomes, the summary screen. Unlike the hero (a one-off headline moment), decision cards sit densely over this backdrop for an entire session, so the filter stays here to keep a busy photo from competing with real UI. No new "diffuse the background" work was needed for the cards themselves — `.panel`/`.liquid-glass` already had `backdrop-filter: blur` and a near-transparent tint from the earlier unified redesign, so switching what sits behind them from a generated canvas to a real photo was enough on its own for a card to visibly blur *that era's* world behind it while staying fully legible.

A three-step "How it works" strip (choose your era → live the day → discover your ending) sits below the Daily Challenge picker for a first-time visitor sizing up the game before committing to one.
