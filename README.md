...
A row of era-switcher tabs beneath the hero copy — one per playable era, name only, active tab underlined — drives which video layer shows, debounced with a 1000ms cooldown so rapid clicks can't overlap transitions. The hero's own "Play Now" button (and the nav's matching pill) starts whichever era is currently showing directly via the existing `selectEra()` flow, into archetype selection — the video hero doubles as the era picker now, not just a banner in front of one. The era-grid further down the page (with its fuller per-era description and the separate Today's Challenge entry point) still exists unchanged for that purpose and for browsing before committing.
