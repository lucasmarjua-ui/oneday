...
assets/era-loop-*.webm         Home hero's per-era video layers (real, self-recorded gameplay clips)
shared/
...
### The video hero and era switcher

The hero band at the very top *does* use video — three short, self-recorded gameplay clips (`assets/era-loop-{greece,neanderthal,future-city}.webm`, 5-8s each, one per playable era, captured the same way as the shared canvas's precursor: Playwright driving a real browser through a seeded day and landing on a recurring-NPC card), stacked as full-bleed `<video>` layers with only the active one at visible opacity (crossfaded via a 1000ms CSS `opacity` transition, never by pausing/restarting playback — every layer autoplays and loops from page load, so switching is instant with no decode stall). They're deliberately blurred, desaturated and dimmed (`filter: blur(1px) saturate(0.7) brightness(0.8)`, `opacity: 0.6` when active) under the same scrim treatment the shared canvas uses elsewhere, so real UI visible in the footage (another era's own HUD) reads as atmospheric texture behind the *actual* hero text, not a second, competing interface.

A row of era-switcher tabs beneath the hero copy — one per playable era, name only, active tab underlined — drives which video layer shows, debounced with a 1000ms cooldown so rapid clicks can't overlap transitions (verified directly in the browser: at every step exactly one `.hero-video-layer` carries the `active` class, matched by a plain `data-era-video === activeSwitcherEraId` string comparison, never by array index). The hero's own "Play Now" button (and the nav's matching pill) starts whichever era is currently showing directly via the existing `selectEra()` flow, into archetype selection — the video hero *is* the era picker now, not a banner in front of one.
...
### The era's video follows you into the day

Picking an era doesn't leave its footage behind on the home screen — `game.html` calls `mountEraVideoBackground(eraId)` (the same `shared/atmosphere-bg.js` module, a sibling to the fog function, never both mounted together) to play that exact era's own clip, at the same dimmed/blurred treatment as the hero, as the backdrop for the whole day: the HUD, every decision card, outcomes, the summary screen. No new "diffuse the background" work was needed for the cards themselves — `.panel`/`.liquid-glass` already had `backdrop-filter: blur` and a near-transparent tint from the earlier unified redesign, so switching what sits behind them from a generated canvas to real footage was enough on its own for a card to visibly blur *that era's* world behind it while staying fully legible; no extra darkening layer behind the card area turned out to be necessary once verified in the browser.

A per-era dark-mode variant for the hero's own text was considered (swapping to a darker tone if a given era's video read as too bright for white text) but turned out unnecessary once actually compared side by side: the same dim/blur/desaturate filter applies uniformly to all three clips, so all three converge to a similarly dark navy tone regardless of the source footage's own palette — white text reads equally well on all three.
