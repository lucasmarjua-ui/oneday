...
import { createDefaultCharacter, findArchetype, renderCharacterSVG } from '../shared/character.js';
...
test('resources use the full universal set, matching Greece rather than Neanderthal\'s fusion', () => {
  ['energy', 'health', 'hunger', 'thirst', 'currency', 'reputation'].forEach(key => {
    assert.ok(key in era.resources, `expected resource "${key}"`);
  });
  assert.ok(!('survival' in era.resources), 'should not use Neanderthal\'s fused survival resource');
});
...
test('every archetype modifier (might, wits, charm) is actually exercised by at least one card', () => {
...
test('every character slot option resolves to a shape the renderer knows about', () => {
  const character = createDefaultCharacter(era);
  era.character.slots.forEach(slot => {
    slot.options.forEach(option => {
      const probe = { ...character, slots: { ...character.slots, [slot.id]: option.id } };
      const svg = renderCharacterSVG(era, probe);
      assert.ok(svg.startsWith('<svg') && svg.endsWith('</svg>'), `${slot.id}/${option.id} did not render a well-formed svg`);
    });
  });
});
