...
test('era.json does not literally reuse Greece\'s memorable flag/counter names', () => {
  const neanderthalNames = new Set([...(era.memories?.flags || []), ...(era.memories?.counters || [])]);
  const greeceNames = new Set([...(greeceEra.memories?.flags || []), ...(greeceEra.memories?.counters || [])]);
  neanderthalNames.forEach(name => assert.ok(!greeceNames.has(name), `memorable name "${name}" is copied verbatim from Greece`));
});
